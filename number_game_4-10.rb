...
def judge_eat_bite(call_array, hand_array)
  ...
  judgement = eat == 3 ? "[3]EAT！ 見事相手の手札を見破りました！" : "[#{eat}]EAT-[#{bite}]BITE"
  puts "#{judgement}\n\n"
...
# コールターン
def call_turn(turn, hand_array, list, name:)
...
a_hand = input_hand(name: "A")
a_hand_array = a_hand.split("")
b_hand = input_hand(name: "B")
...
while true
  a_call = call_turn(turn, b_hand_array, a_called_list, name: "A")
  a_called_list.push(a_call)
  b_call = call_turn(turn, a_hand_array, b_called_list, name: "B")
